unsafe extern "C" {
    fn rust_mcil_dotnet_path_combine_utf8_len(
        left_ptr: *const u8,
        left_len: i64,
        right_ptr: *const u8,
        right_len: i64,
    ) -> i32;
    fn rust_mcil_dotnet_path_copy_combine_utf8(
        left_ptr: *const u8,
        left_len: i64,
        right_ptr: *const u8,
        right_len: i64,
...
#[unsafe(no_mangle)]
pub extern "C" fn dotnet_runtime_path_combine_score() -> i32 {
    let left = concat!(env!("CARGO_MANIFEST_DIR"), "\\..\\std_fs\\fixtures");
    let right = "input.txt";
    let extension = ".data";
    let old = "in";
    let new = "out";
    ...
    let combined_len = unsafe {
        rust_mcil_dotnet_path_combine_utf8_len(
            left.as_ptr(),
            left.len() as i64,
            right.as_ptr(),
            right.len() as i64,
    ...
    let combined_written = unsafe {
        rust_mcil_dotnet_path_copy_combine_utf8(
            left.as_ptr(),
            left.len() as i64,
            right.as_ptr(),
            right.len() as i64,
    ...
    let combined_file_name_len = unsafe {
        rust_mcil_dotnet_path_get_file_name_len(combined_path.as_ptr(), combined_written as i64)
    };
    ...
    combined_file_name_len * 1000 + file_name_written * 100 + transformed_written * 10 + contains * 10 + index
}