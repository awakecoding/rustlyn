unsafe extern "C" {
    fn rustlyn_dotnet_path_combine_utf8_len(
        left_ptr: *const u8,
        left_len: i64,
        right_ptr: *const u8,
        right_len: i64,
    ) -> i32;
    fn rustlyn_dotnet_path_copy_combine_utf8(
        left_ptr: *const u8,
        left_len: i64,
        right_ptr: *const u8,
        right_len: i64,
...
pub extern "C" fn dotnet_runtime_path_combine_score() -> i32 {
    ...
    let combined_len = unsafe {
        rustlyn_dotnet_path_combine_utf8_len(
            left.as_ptr(),
            left.len() as i64,
            right.as_ptr(),
            right.len() as i64,
    ...
    let combined_written = unsafe {
        rustlyn_dotnet_path_copy_combine_utf8(
            left.as_ptr(),
            left.len() as i64,
            right.as_ptr(),
            right.len() as i64,
    ...
    let combined_file_name_len = unsafe {
        rustlyn_dotnet_path_get_file_name_len(combined_path.as_ptr(), combined_written as i64)
    };
