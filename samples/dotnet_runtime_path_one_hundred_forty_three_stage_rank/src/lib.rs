...
#[unsafe(no_mangle)]
pub extern "C" fn dotnet_runtime_path_one_hundred_forty_three_stage_rank_score() -> i32 {
    ...
    let lemnoidette_old = "spheroidette";
    let lemnoidette_new = "lemnoidette";
    let lemnoidette_needle = "lemnoidette";
    let enneadecawake_extension = ".enneadecawake";
    let enneadecawake_needle = "enneadecawake";
    let chenillecord_old = "tambourcord";
    let chenillecord_new = "chenillecord";
    let chenillecord_needle = "chenillecord";
    let ovaloidette_old = "lemnoidette";
    let ovaloidette_new = "ovaloidette";
    let ovaloidette_needle = "ovaloidette";
    let icosawake_extension = ".icosawake";
    let icosawake_needle = "icosawake";
    let couchingcord_old = "chenillecord";
    let couchingcord_new = "couchingcord";
    let couchingcord_needle = "couchingcord";
    ...
    let mut selected_spheroidette_ptr = spheroidette_value.as_ptr();
    let mut selected_spheroidette_written = spheroidette_value_written;
    if selected_spheroidette_index == 1 {
        selected_spheroidette_ptr = octadecawake_value.as_ptr();
        selected_spheroidette_written = octadecawake_value_written;
    } else if selected_spheroidette_index == 2 {
        selected_spheroidette_ptr = tambourcord_value.as_ptr();
        selected_spheroidette_written = tambourcord_value_written;
    }

    let lemnoidette_value_len = unsafe {
        rustlyn_dotnet_string_replace_utf8_len(
            selected_spheroidette_ptr,
            selected_spheroidette_written as i64,
            lemnoidette_old.as_ptr(),
            lemnoidette_old.len() as i64,
            lemnoidette_new.as_ptr(),
            lemnoidette_new.len() as i64,
        )
    };
    let mut lemnoidette_value = vec![0u8; lemnoidette_value_len as usize];
    let lemnoidette_value_written = unsafe {
        rustlyn_dotnet_string_copy_replace_utf8(
            selected_spheroidette_ptr,
            selected_spheroidette_written as i64,
            lemnoidette_old.as_ptr(),
            ...
            lemnoidette_new.as_ptr(),
            lemnoidette_new.len() as i64,
            lemnoidette_value.as_mut_ptr(),
            lemnoidette_value.len() as i64,
        )
    };
    let lemnoidette_contains = unsafe {
        rustlyn_dotnet_string_contains(
            lemnoidette_value.as_ptr(),
            lemnoidette_value_written as i64,
            lemnoidette_needle.as_ptr(),
            lemnoidette_needle.len() as i64,
        )
    };

    let enneadecawake_value_len = unsafe {
        rustlyn_dotnet_path_change_extension_utf8_len(
            selected_spheroidette_ptr,
            selected_spheroidette_written as i64,
            enneadecawake_extension.as_ptr(),
            enneadecawake_extension.len() as i64,
        )
    };
    let mut enneadecawake_value = vec![0u8; enneadecawake_value_len as usize];
    let enneadecawake_value_written = unsafe {
        rustlyn_dotnet_path_copy_change_extension_utf8(
            selected_spheroidette_ptr,
            selected_spheroidette_written as i64,
            enneadecawake_extension.as_ptr(),
            enneadecawake_extension.len() as i64,
            enneadecawake_value.as_mut_ptr(),
            enneadecawake_value.len() as i64,
        )
    };
    let enneadecawake_index = unsafe {
        rustlyn_dotnet_string_index_of(
            enneadecawake_value.as_ptr(),
            enneadecawake_value_written as i64,
            enneadecawake_needle.as_ptr(),
            enneadecawake_needle.len() as i64,
        )
    };

    let chenillecord_source_len = unsafe {
        rustlyn_dotnet_path_get_file_name_without_extension_utf8_len(
            selected_spheroidette_ptr,
            selected_spheroidette_written as i64,
        )
    };
    let mut chenillecord_source = vec![0u8; chenillecord_source_len as usize];
    let chenillecord_source_written = unsafe {
        rustlyn_dotnet_path_copy_file_name_without_extension_utf8(
            selected_spheroidette_ptr,
            selected_spheroidette_written as i64,
            chenillecord_source.as_mut_ptr(),
            chenillecord_source.len() as i64,
        )
    };
    let chenillecord_value_len = unsafe {
        rustlyn_dotnet_string_replace_utf8_len(
            chenillecord_source.as_ptr(),
            chenillecord_source_written as i64,
            chenillecord_old.as_ptr(),
            chenillecord_old.len() as i64,
            chenillecord_new.as_ptr(),
            chenillecord_new.len() as i64,
        )
    };
    let mut chenillecord_value = vec![0u8; chenillecord_value_len as usize];
    let chenillecord_value_written = unsafe {
        rustlyn_dotnet_string_copy_replace_utf8(
            chenillecord_source.as_ptr(),
            chenillecord_source_written as i64,
            chenillecord_old.as_ptr(),
            ...
            chenillecord_new.as_ptr(),
            chenillecord_new.len() as i64,
            chenillecord_value.as_mut_ptr(),
            chenillecord_value.len() as i64,
        )
    };
    let chenillecord_contains = unsafe {
        rustlyn_dotnet_string_contains(
            chenillecord_value.as_ptr(),
            chenillecord_value_written as i64,
            chenillecord_needle.as_ptr(),
            chenillecord_needle.len() as i64,
        )
    };

    let mut selected_lemnoidette_index = 0i32;
    let mut best_lemnoidette_score = i32::MIN;
    let mut lemnoidette_index = 0i32;
    while lemnoidette_index < 3 {
        let mut candidate_lemnoidette_score =
            lemnoidette_value_written * 10 + lemnoidette_contains * 50;
        if lemnoidette_index == 1 {
            candidate_lemnoidette_score = enneadecawake_value_written * 10 + enneadecawake_index;
        } else if lemnoidette_index == 2 {
            candidate_lemnoidette_score =
                chenillecord_value_written * 10 + chenillecord_contains * 50;
        }

        let mut lemnoidette_bonus = 0i32;
        if lemnoidette_index == selected_spheroidette_index {
            lemnoidette_bonus += 25;
        }
        if lemnoidette_index == selected_campyloidette_index {
            lemnoidette_bonus += 15;
        }
        if lemnoidette_index == selected_epicycloidette_index {
            lemnoidette_bonus += 5;
        }
        if lemnoidette_index == 0 && lemnoidette_contains != 0 {
            lemnoidette_bonus += 20;
        }
        if lemnoidette_index == 1 && enneadecawake_index >= 0 {
            lemnoidette_bonus += 10;
        }
        if lemnoidette_index == 2 && chenillecord_contains != 0 {
            lemnoidette_bonus += 30;
        }

        let lemnoidette_score = candidate_lemnoidette_score + lemnoidette_bonus;
        if lemnoidette_score > best_lemnoidette_score {
            best_lemnoidette_score = lemnoidette_score;
            selected_lemnoidette_index = lemnoidette_index;
        }

        lemnoidette_index += 1;
    }

    let mut selected_lemnoidette_ptr = lemnoidette_value.as_ptr();
    let mut selected_lemnoidette_written = lemnoidette_value_written;
    if selected_lemnoidette_index == 1 {
        selected_lemnoidette_ptr = enneadecawake_value.as_ptr();
        selected_lemnoidette_written = enneadecawake_value_written;
    } else if selected_lemnoidette_index == 2 {
        selected_lemnoidette_ptr = chenillecord_value.as_ptr();
        selected_lemnoidette_written = chenillecord_value_written;
    }

    let ovaloidette_value_len = unsafe {
        rustlyn_dotnet_string_replace_utf8_len(
            selected_lemnoidette_ptr,
            selected_lemnoidette_written as i64,
            ovaloidette_old.as_ptr(),
            ovaloidette_old.len() as i64,
            ovaloidette_new.as_ptr(),
            ovaloidette_new.len() as i64,
        )
    };
    let mut ovaloidette_value = vec![0u8; ovaloidette_value_len as usize];
    let ovaloidette_value_written = unsafe {
        rustlyn_dotnet_string_copy_replace_utf8(
            selected_lemnoidette_ptr,
            selected_lemnoidette_written as i64,
            ovaloidette_old.as_ptr(),
            ...
            ovaloidette_new.as_ptr(),
            ovaloidette_new.len() as i64,
            ovaloidette_value.as_mut_ptr(),
            ovaloidette_value.len() as i64,
        )
    };
    let ovaloidette_contains = unsafe {
        rustlyn_dotnet_string_contains(
            ovaloidette_value.as_ptr(),
            ovaloidette_value_written as i64,
            ovaloidette_needle.as_ptr(),
            ovaloidette_needle.len() as i64,
        )
    };

    let icosawake_value_len = unsafe {
        rustlyn_dotnet_path_change_extension_utf8_len(
            selected_lemnoidette_ptr,
            selected_lemnoidette_written as i64,
            icosawake_extension.as_ptr(),
            icosawake_extension.len() as i64,
        )
    };
    let mut icosawake_value = vec![0u8; icosawake_value_len as usize];
    let icosawake_value_written = unsafe {
        rustlyn_dotnet_path_copy_change_extension_utf8(
            selected_lemnoidette_ptr,
            selected_lemnoidette_written as i64,
            icosawake_extension.as_ptr(),
            icosawake_extension.len() as i64,
            icosawake_value.as_mut_ptr(),
            icosawake_value.len() as i64,
        )
    };
    let icosawake_index = unsafe {
        rustlyn_dotnet_string_index_of(
            icosawake_value.as_ptr(),
            icosawake_value_written as i64,
            icosawake_needle.as_ptr(),
            icosawake_needle.len() as i64,
        )
    };

    let couchingcord_source_len = unsafe {
        rustlyn_dotnet_path_get_file_name_without_extension_utf8_len(
            selected_lemnoidette_ptr,
            selected_lemnoidette_written as i64,
        )
    };
    let mut couchingcord_source = vec![0u8; couchingcord_source_len as usize];
    let couchingcord_source_written = unsafe {
        rustlyn_dotnet_path_copy_file_name_without_extension_utf8(
            selected_lemnoidette_ptr,
            selected_lemnoidette_written as i64,
            couchingcord_source.as_mut_ptr(),
            couchingcord_source.len() as i64,
        )
    };
    let couchingcord_value_len = unsafe {
        rustlyn_dotnet_string_replace_utf8_len(
            couchingcord_source.as_ptr(),
            couchingcord_source_written as i64,
            couchingcord_old.as_ptr(),
            couchingcord_old.len() as i64,
            couchingcord_new.as_ptr(),
            couchingcord_new.len() as i64,
        )
    };
    let mut couchingcord_value = vec![0u8; couchingcord_value_len as usize];
    let couchingcord_value_written = unsafe {
        rustlyn_dotnet_string_copy_replace_utf8(
            couchingcord_source.as_ptr(),
            couchingcord_source_written as i64,
            couchingcord_old.as_ptr(),
            ...
            couchingcord_new.as_ptr(),
            couchingcord_new.len() as i64,
            couchingcord_value.as_mut_ptr(),
            couchingcord_value.len() as i64,
        )
    };
    let couchingcord_contains = unsafe {
        rustlyn_dotnet_string_contains(
            couchingcord_value.as_ptr(),
            couchingcord_value_written as i64,
            couchingcord_needle.as_ptr(),
            couchingcord_needle.len() as i64,
        )
    };

    let mut selected_ovaloidette_index = 0i32;
    let mut best_ovaloidette_score = i32::MIN;
    let mut ovaloidette_index = 0i32;
    while ovaloidette_index < 3 {
        let mut candidate_ovaloidette_score =
            ovaloidette_value_written * 10 + ovaloidette_contains * 50;
        if ovaloidette_index == 1 {
            candidate_ovaloidette_score = icosawake_value_written * 10 + icosawake_index;
        } else if ovaloidette_index == 2 {
            candidate_ovaloidette_score =
                couchingcord_value_written * 10 + couchingcord_contains * 50;
        }

        let mut ovaloidette_bonus = 0i32;
        if ovaloidette_index == selected_lemnoidette_index {
            ovaloidette_bonus += 25;
        }
        if ovaloidette_index == selected_spheroidette_index {
            ovaloidette_bonus += 15;
        }
        if ovaloidette_index == selected_campyloidette_index {
            ovaloidette_bonus += 5;
        }
        if ovaloidette_index == 0 && ovaloidette_contains != 0 {
            ovaloidette_bonus += 20;
        }
        if ovaloidette_index == 1 && icosawake_index >= 0 {
            ovaloidette_bonus += 10;
        }
        if ovaloidette_index == 2 && couchingcord_contains != 0 {
            ovaloidette_bonus += 30;
        }

        let ovaloidette_score = candidate_ovaloidette_score + ovaloidette_bonus;
        if ovaloidette_score > best_ovaloidette_score {
            best_ovaloidette_score = ovaloidette_score;
            selected_ovaloidette_index = ovaloidette_index;
        }
    ...
    (selected_directory_index + 1) * 100000000
        ...
        + (selected_lemnoidette_index + 1) * 10
        + (selected_ovaloidette_index + 1)
}