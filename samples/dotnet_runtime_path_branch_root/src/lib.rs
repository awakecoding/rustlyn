...
#[unsafe(no_mangle)]
pub extern "C" fn dotnet_runtime_path_branch_root_score() -> i32 {
    ...
    let choose_temp = temp_contains != 0 && documents_contains == 0;
    let (selected_file_name_ptr, selected_file_name_written, branch_score) = if choose_temp {
        (temp_file_name.as_ptr(), temp_file_name_written, 1)
    } else {
        (documents_file_name.as_ptr(), documents_file_name_written, 2)
    };
    ...
    branch_score * 100000000
        + merged_full_written * 1000000
        + selected_file_name_written * 10000
