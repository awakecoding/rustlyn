unsafe extern "C" {
    fn rust_mcil_dotnet_user_profile_utf8_len() -> i32;
    fn rust_mcil_dotnet_copy_user_profile_utf8(
...
pub extern "C" fn dotnet_runtime_user_profile_score() -> i32 {
    ...
    let root_len = unsafe { rust_mcil_dotnet_user_profile_utf8_len() };
    let mut root = vec![0u8; root_len as usize];
    let root_written = unsafe {
        rust_mcil_dotnet_copy_user_profile_utf8(root.as_mut_ptr(), root.len() as i64)
    };
