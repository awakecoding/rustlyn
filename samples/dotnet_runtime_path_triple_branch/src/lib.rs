...
#[unsafe(no_mangle)]
pub extern "C" fn dotnet_runtime_path_triple_branch_score() -> i32 {
    ...
    let (selected_file_name_ptr, selected_file_name_written, branch_score) = if temp_index > current_index {
        (temp_file_name.as_ptr(), temp_file_name_written, 2)
    } else if documents_contains != 0 && temp_index == current_index {
