...
| --- | --- | --- | --- | --- |
| SDK-style `.rsproj` build with `dotnet build` | `artifacts/sdk-0.1.5/extracted/Sdk/`, `artifacts/sdk-0.1.5/extracted/build/` | `RustMcil.Sdk` MSBuild facade | Started with a thin local and packable SDK that bundles `RustMcil.Tool`, delegates to `translate`, and validates library, explicit/inferred binary, build-std, generated-bindings workload, copied support assemblies, SourceGear-style property aliases, and generated Cargo metadata | Add richer cargo project metadata features before publishing as a real NuGet SDK |
| Synthetic Cargo project generation from project metadata | `artifacts/decompiled/rsbuild-program/Program.decompiled.cs` | `RustMcil.Sdk` MSBuild facade plus cargo/backend driver | Started with a generated `Cargo.toml` under `obj/.../rs` for `.rsproj` projects with `src/lib.rs` or `src/main.rs`, `RustEdition`, local `RustReference` path dependencies, and crates.io `RustCrateReference` version/default-feature/feature-list metadata | Decide when multi-crate bitcode linking is needed for non-inlined dependency calls |
