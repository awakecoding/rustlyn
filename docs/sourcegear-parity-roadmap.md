...
| --- | --- | --- | --- | --- |
| SDK-style `.rsproj` build with `dotnet build` | `artifacts/sdk-0.1.5/extracted/Sdk/`, `artifacts/sdk-0.1.5/extracted/build/` | `RustMcil.Sdk` MSBuild facade | Started with a thin local and packable SDK that bundles `RustMcil.Tool`, delegates to `translate`, and validates library, explicit/inferred binary, build-std, generated-bindings workload, copied support assemblies, and SourceGear-style property aliases | Add richer cargo project metadata features before publishing as a real NuGet SDK |
