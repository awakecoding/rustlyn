...
| Synthetic Cargo project generation from project metadata | `artifacts/decompiled/rsbuild-program/Program.decompiled.cs` | `RustMcil.Sdk` MSBuild facade plus cargo/backend driver | Started with a generated `Cargo.toml` under `obj/.../rs` for `.rsproj` projects with `src/lib.rs` or `src/main.rs`, `RustEdition`, local `RustReference` path dependencies, and crates.io `RustCrateReference` version/default-feature metadata | Add `RustCrateReference` feature-list metadata and decide when multi-crate bitcode linking is needed |
